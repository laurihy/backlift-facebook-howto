...
App.databrowser.colview = Backbone.View.extend({
  ...
  render: function(){
    this.$el.html(_.template(this.template,
      {formplaceholder: this.formatPlaceholder(),
    ...
    var schemaeditview = App.databrowser.viewGen('schemaedit',
                                                App.databrowser.schemaEditView,
                                                {'collection':this.collection});
    //schemaeditview.$el = $('#collectionschema');
    //schemaeditview.render();
    //schemaeditview.delegateEvents()
    this.renderItems();
  },
  ...
  renderItems: function(){
    ...
    holder.append('<p><span>Showing '+col.length+' documents:</span></p>')
  ...
  formatSchema: function(){
    return this.collection.schema; //JSON.stringify(this.collection.schema,null,' ')
  },
...
App.databrowser.schemaEditView = Backbone.View.extend({
  template: $('#schemaedit-view').html(),
  initialize: function(options){
    this.collection = options.collection;
  },
  ...
  render: function(){
    this.$el.html(_.template(this.template, {
      ...
      dataTypes: this.collection.dataTypes
    }));
  },

  parseSchemaData: function(){
    ret = {}
    this.$el.find('table').find('tr').each(function(){
      var row = $(this);
      var fieldname = row.find('input[name="fieldname"]').val();
      var type = row.find('select[name="type"]').val();
      var rules = row.find('input[name="rules"]').val();
      var required = row.find('input[name="required"]').is(':checked');
      if(fieldname){
        ret[fieldname] = {
          'type': type,
          'required': required
  ...
  updateSchema: function(newschema){
    data = '{"collections": {"'+this.collection.getName()+'": '+JSON.stringify(newschema)+'}}'
    var self = this;
    $.ajax({
      type:'POST',
      ...
      contentType: 'application/json',
      success: function(data) {
        self.updateSuccess(newschema);
      },
      error: function(response) {
        alert('There was an error, please fix them')
      }
    });
  },

  updateSuccess: function(data){
    this.collection.schema = data;
    var newrow = this.$el.find('tr.newrow').first().clone()
    newrow.find(':input').each(function(){
      $(this).val('');
    })
    $('#collectionschema').find('tr.newrow').each(function(){
      if($(this).find('input[name="fieldname"]').val()===''){
        $(this).remove()
      } else {
        $(this).removeClass('newrow');
        $(this).find('input[name="fieldname"]').attr('disabled','disabled')
      }
    });
    this.$el.find('table').append(newrow);
  },
  ...
  events: {
    ...
    "submit #collectionschema form": function(e){
      e.preventDefault();
      this.updateSchema(this.parseSchemaData());
    }
  }
